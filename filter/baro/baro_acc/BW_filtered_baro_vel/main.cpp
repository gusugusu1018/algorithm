...
main() {
	...
	float	baro_alt_old;
	float	baro_vel;
	float	filtered_vel_old;
	float	filtered_vel_old2;
	...
	while (std::getline(ifs,str)) {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (i == 0) {
				...
				if (initial_flag) {
					...
					filtered_vel_old = 0.0f;
					filtered_vel_old2 = 0.0f;
					initial_flag = false;
				} else {
					...
					// BW_filter
					baro_vel = BW_filter(filtered_vel_old,filtered_vel_old2,baro_vel,BARO_FILTER_FC,BARO_FILTER_TAU);
				}
			}
		}
		// print
		...
		oldtime = newtime;
		baro_alt_old = baro_alt;
		filtered_vel_old2 = filtered_vel_old;
		filtered_vel_old = baro_vel;
	}
	return 0;
}
