...
double backward(dataset data
	...
	double cost=forward(data,wh,bh,zh,ah,wo,bo,zo,ao);
	for (int i=0;i<OUTPUTNO;i++) {
...
int main()
{
	...
	for (int counter=0;counter<epoch;counter++) {
		for (int n=0;n<NMAX;n++) {
			cost=backward(data[n],wh,bh,zh,ah[n],wo,bo,zo,ao[n],deltah[n],deltao[n]);
		}
